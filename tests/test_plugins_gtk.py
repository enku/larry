...
class GtkTests(IsolatedAsyncioTestCase):
    async def test(self, write_text_file, fixtures: Fixtures) -> None:
        output = io.BytesIO()
        write_text_file.side_effect = lib.mock_write_text_file(output)
        ...
        await gtk.plugin(colors, configmaker.config["plugins:gtk"])
